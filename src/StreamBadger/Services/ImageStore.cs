...
namespace StreamBadger.Services
{
    ...
    {
        private readonly ConcurrentDictionary<string, ImageModel?> _cache = new(StringComparer.OrdinalIgnoreCase);
        private readonly string _baseDirectory;

        public ImageStore()
        {
            var appData = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
            _baseDirectory = Path.Combine(appData, "StreamBadger", "Images");
        }
